...
class HypergraphDirectedLineKernel:
    ...
    def directed_line_expansion(self, hg_list, e_lbl):
        ...
        for hg_idx, hg in enumerate(hg_list):
            ...
            dl_list.append({"num_v": num_v, "e_list": e_list, "v_lbl": v_lbl, "dhg": dl})
        return dl_list
        ...
        for _ in range(self.n_iter):
            for dl in dl_list:
                ...
                for v_idx in range(dl["num_v"]):
                    ...
                    in_nbr_lbl = sorted([dl["in_v_lbl"][u_idx] for u_idx in dl["dhg"].nbr_v_in(v_idx)])
                    out_cur_lbl = dl["out_v_lbl"][v_idx]
                    out_nbr_lbl = sorted([dl["out_v_lbl"][u_idx] for u_idx in dl["dhg"].nbr_v_out(v_idx)])
        ...
        for _ in range(self.n_iter):
            for hg in dl_list:
                ...
                for v_idx in range(hg["num_v"]):
                    ...
                    in_nbr_lbl = sorted([hg["in_v_lbl"][u_idx] for u_idx in hg["dhg"].nbr_v_in(v_idx)])
                    out_cur_lbl = hg["out_v_lbl"][v_idx]
                    out_nbr_lbl = sorted([hg["out_v_lbl"][u_idx] for u_idx in hg["dhg"].nbr_v_out(v_idx)])
                    in_tmp.append(f"{in_cur_lbl},{in_nbr_lbl}")
                    out_tmp.append(f"{out_cur_lbl},{out_nbr_lbl}")
                hg["in_v_lbl"] = in_tmp
