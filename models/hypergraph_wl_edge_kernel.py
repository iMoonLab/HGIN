...
# Hypergraph WL Edge Kernel from "Feng et al. 2023"
class HypergraphWLEdgeKernel:
    ...
    def remap_e(self, hg_list, cnt, drop=False):
        ...
        for hg_idx, hg in enumerate(hg_list):
            for e_idx in range(hg["dhg"].num_e):
                cur_nbr_lbl = str(sorted(hg["v_lbl"][v_idx] for v_idx in hg["dhg"].nbr_v(e_idx)))
    ...
    def cnt2mat(self, cnt):
        ...
        return (
            torch.sparse_coo_tensor(
                torch.tensor([row_idx, col_idx]), torch.tensor(data), size=(len(cnt), len(self._e_map))
            )
            .coalesce()
            .float()
        )
