...
class HypergraphRootedKernel:
    ...
    def count2mat(self, count, l):
        ...
        return (
            torch.sparse_coo_tensor(
                torch.tensor([row_idx, col_idx]), torch.tensor(data), size=(len(count), len(self._seq_map[l]))
            )
            .coalesce()
            .float()
        )
    ...
    def fit_transform(self, hg_list):
        ...
        self._count = {idx: [defaultdict(int) for _ in range(len(hg_list))] for idx in range(1, self.max_walk_len + 1)}
        ...
        for hg_idx, hg in enumerate(hg_list):
            ...
            if self.n_seq == -1:
                n_seq = int(
                    hg["dhg"].num_e * (np.mean(hg["dhg"].deg_e) * np.mean(hg["dhg"].deg_v)) ** self.max_walk_len / 200
                )
            else:
                n_seq = self.n_seq
            num_e = hg["dhg"].num_e
        ...
        self.raw_train_ft = {
            l: self.raw_train_cnt[l].mm(self.raw_train_cnt[l].t()).to_dense() for l in self.raw_train_cnt
        }
    ...
    def transform(self, hg_list):
        # initialize the sequence count
        count = {idx: [defaultdict(int) for _ in range(len(hg_list))] for idx in range(1, self.max_walk_len + 1)}
        # initialize the hyperedge feature
        if not self.degree_as_label:
            e_lbl = [hg["e_lbl"] for hg in hg_list]
        else:
        ...
        raw_test_ft = {l: raw_test_cnt[l].mm(self.raw_train_cnt[l].t()).to_dense() for l in raw_test_cnt}
        test_ft = self.fuse(raw_test_ft)
        if self.normalize:
            test_ft_diag = self.fuse(
                {l: torch.sparse.sum(tc * tc, dim=1).to_dense() for l, tc in raw_test_cnt.items()}
            )
