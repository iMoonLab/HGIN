...
class GraphLet:
    ...
    def sub(self, sub_v):
        ...
        sub_adj_list = {v_map[v]: set([v_map[u] for u in self.adj_list[v] & sub_v]) for v in sub_v}
        return GraphLet.from_adj_list(sub_adj_list)

    def permute_adj(self, perm):
        v_map = {v: i for i, v in enumerate(perm)}
        perm_adj_list = {v_map[v]: set([v_map[u] for u in self.adj_list[v]]) for v in range(self.num_v)}
...
class GraphletSampling:
    def __init__(self, k=5, normalize=True, sampling=None, symmetric=True):
        ...
        if sampling is None:
            ...
            a = a_map[k]
            self.n_samples = math.ceil(2 * (a * np.log10(2) + np.log10(1 / delta)) / (epsilon**2) / 200)
    ...
    def count2mat(self, count):
        ...
        return (
            torch.sparse_coo_tensor(
                torch.tensor([row_idx, col_idx]), torch.tensor(data), size=(len(count), len(self._graph_bins))
            )
            .coalesce()
            .float()
        )
    ...
    def unionProduct(S1, S2):
        ...
        if not len(S1):
            ...
            return {
                ...
                and (any(MarkN[j] for j in s2) or all(not len({treeL[j] for j in tree[i]} & s2p) for i in s1))
            }
